...
blender_path = os.path.join("C:\\Program Files (x86)\\Steam\\steamapps\\common\\Blender", universal_blender_path)
alternate_blender_path = os.path.join(f"C:\\Program Files\\Blender Foundation\\Blender {blender_version}", universal_blender_path)
...
if not os.path.exists(blender_path) and not os.path.exists(alternate_blender_path):
    ...
    print(error_message)
    blender_path = os.path.join(get_input(get_localized_message("Enter the path to Blender folder manually (e.g., C:\\Downloads\\Blender\\4.3): ", ["Укажите путь до папки Blender вручную (например, C:\\Downloads\\Blender\\4.3): "])), universal_blender_path)
